...
from ...serializers import InterviewsSerializer, ContextError, NotFound
...
class InterviewsSerializerTests(Neo4jTestCase):
    ...
    def test_get_extraits_and_tags_urls(self):
        serializer = InterviewsSerializer(self.interview, context={'request': self.request})
    ...
    def test_create_raises_contexterror_without_extrait(self):
        serializer = InterviewsSerializer(data={'uuid': self.interview.uuid, 'position': 1}, context={'request': self.request})
        self.assertTrue(serializer.is_valid(), serializer.errors)
    ...
    def test_delete_disconnects_interview_from_extrait(self):
        ...
        serializer = InterviewsSerializer(context={'request': self.request, 'extrait': self.extrait})
