...
class ExtraitSerializerTests(Neo4jTestCase):
    ...
    def test_init_with_interview_keeps_position_field(self):
        ...
        serializer = ExtraitSerializer(context={'request': self.request, 'interview': interview})
    ...
    def test_get_position_returns_correct_value(self):
        ...
        serializer = ExtraitSerializer(extrait, context={'request': self.request, 'interview': interview})
        data = serializer.data
    ...
    def test_update_success_updates_fields_and_relations(self):
        ...
        serializer = ExtraitSerializer(instance=extrait, data=payload, context={'request': self.request})
        self.assertTrue(serializer.is_valid(), serializer.errors)
    ...
    def test_update_with_nonexistent_question_raises_notfound(self):
        ...
        serializer = ExtraitSerializer(instance=extrait, data=payload, context={'request': self.request}, partial=True)
        self.assertTrue(serializer.is_valid(), serializer.errors)
        with self.assertRaises(NotFound):
            serializer.save()
    ...
    def test_update_raises_uniqueproperty_on_duplicate(self):
        ...
        serializer = ExtraitSerializer(instance=e2, data=payload, context={'request': self.request}, partial=True)
        self.assertTrue(serializer.is_valid(), serializer.errors)
        with self.assertRaises(ValidatorUnique):
            serializer.save()
    ...
    def test_update_disconnect_question_ignores_when_none(self):
        ...
        extrait = Extrait(titre="NoQuestion", youtube_url="ytx", vimeo_url="vmx", duree=15).save()
