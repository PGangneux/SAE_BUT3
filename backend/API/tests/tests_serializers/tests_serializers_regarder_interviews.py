...
class RegarderInterviewsSerializerTests(Neo4jTestCase):
    ...
    def test_getters_return_correct_data(self):
        ...
        serializer = RegarderInterviewsSerializer(self.interview, context={'request': self.request, 'utilisateur': self.user})
    ...
    def test_get_date_heure_raises_contexterror_without_utilisateur(self):
        serializer = RegarderInterviewsSerializer(self.interview, context={'request': self.request})
    ...
    def test_create_raises_contexterror_without_utilisateur(self):
        serializer = RegarderInterviewsSerializer(data={'uuid': self.interview.uuid}, context={'request': self.request})
        self.assertTrue(serializer.is_valid(), serializer.errors)
        with self.assertRaises(ContextError):
            serializer.save()
    ...
    def test_delete_disconnects_interview_from_utilisateur(self):
        self.user.regarder_interviews.connect(self.interview)
        serializer = RegarderInterviewsSerializer(context={'utilisateur': self.user, 'request': self.request})
