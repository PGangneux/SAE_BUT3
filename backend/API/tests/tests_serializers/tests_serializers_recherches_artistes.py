...
from ...serializers import RecherchesArtistesSerializer, ContextError, NotFound
...
class RecherchesArtistesSerializerTests(Neo4jTestCase):
    ...
    def test_getters_return_correct_data(self):
        ...
        serializer = RecherchesArtistesSerializer(self.artiste, context={'request': self.request, 'utilisateur': self.user})
    ...
    def test_get_date_heure_raises_contexterror_without_utilisateur(self):
        serializer = RecherchesArtistesSerializer(self.artiste, context={'request': self.request})
        with self.assertRaises(ContextError):
            serializer.get_date_heure(self.artiste)

    # --- create ---
    def test_create_connects_artiste_to_utilisateur(self):
        serializer = RecherchesArtistesSerializer(data={'uuid': self.artiste.uuid}, context={'utilisateur': self.user, 'request': self.request})
    ...
    def test_create_raises_contexterror_without_utilisateur(self):
        serializer = RecherchesArtistesSerializer(data={'uuid': self.artiste.uuid}, context={'request': self.request})
        self.assertTrue(serializer.is_valid(), serializer.errors)
        with self.assertRaises(ContextError):
            serializer.save()
    ...
    def test_delete_disconnects_artiste_from_utilisateur(self):
        self.user.recherches_artistes.connect(self.artiste)
        serializer = RecherchesArtistesSerializer(context={'utilisateur': self.user, 'request': self.request})
