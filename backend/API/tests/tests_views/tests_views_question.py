...
class QuestionViewSetAPITests(Neo4jTestCase):
    def setUp(self):
        ...
        self.question1 = Question(
            texte=f"Question unique {uuid4()}"
        ).save()
        self.question2 = Question(
            texte=f"Question autre {uuid4()}"
        ).save()
    ...
    def test_retrieve_nonexistent_question(self):
        url = reverse('question-detail', kwargs={'uuid': '00000000-0000-0000-0000-000000000000'})
        response = self.client.get(url)
...
class ThemeQuestionViewSetAPITests(Neo4jTestCase):
    def setUp(self):
        ...
        self.theme = Theme(
            name=f"Theme_{uuid4()}",
            description="Description thème"
        ).save()

        # Création de deux questions, seule l'une d'elles sera liée au thème
        self.question_linked = Question(
            texte="Question liée au thème"
        ).save()
        self.question_unlinked = Question(
            texte="Question non liée"
        ).save()
    ...
    def test_retrieve_question_by_theme(self):
        url = reverse('question-detail', kwargs={
            ...
            'uuid': self.question_linked.uuid
        })
        response = self.client.get(url)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
